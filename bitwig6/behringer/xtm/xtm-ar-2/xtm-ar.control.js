...
function handleUpperButton(buttonIndex, isPressed) {
    ...
    if (currentMode === MODE.MIXER) {
        selectChildTrackExclusive(buttonIndex);
...
function selectChildTrackExclusive(childIndex) {
    if (!pinnedTrackIsGroup || !childTrackBank) {
        if (DEBUG) {
            host.println(`Cannot select child ${childIndex} - not in group mode`);
        }
    ...
    if (DEBUG) {
        host.println(`=== Exclusive select: child ${childIndex} (${childTrack.name().get()}) ===`);
    }
    
    for (let i = 0; i < 8; i++) {
        if (childTracks[i] && childTracks[i].exists().get()) {
            const shouldMute = (i !== childIndex);
            childTrackMutes[i].set(shouldMute);
...
function updateUpperButtonLEDs() {
    ...
    if (currentMode === MODE.MIXER) {
        for (let i = 0; i < 8; i++) {
            if (childTracks[i] && childTracks[i].exists().get()) {
                const isMuted = childTrackMutes[i].get();
                const ledState = isMuted ? LED_STATE.OFF : LED_STATE.ON;
...
function setupTracks() {
    ...
    try {
        ...
        childTrackBank = pinnedTrack.createMainTrackBank(8, 0, 8, false);
        ...
        for (let i = 0; i < 8; i++) {
            ...
            const mute = childTrack.mute();
            mute.markInterested();
            childTrackMutes[i] = mute;
...
function setupTrackObservers() {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        childTrackMutes[i].addValueObserver(function(isMuted) {
            if (pinnedTrackIsGroup && currentMode === MODE.MIXER) {
                const ledState = isMuted ? LED_STATE.OFF : LED_STATE.ON;
                setUpperButtonLED(childIndex, ledState);
            }
        });
    }
}
